...
namespace Better.Commons.Runtime.Utilities
{
	...
	{
		public static void LogCurrentPlayerLoopTypes(string message, int sourceDepth, LogType logType = LogType.Log)
		{
			var stringBuilder = new StringBuilder();
			stringBuilder.AppendLine(message);

			var currentLoop = PlayerLoop.GetCurrentPlayerLoop();
			currentLoop.CollectDebugInfo(sourceDepth, ref stringBuilder);
		...
		{
			var sourceDepth = DebugUtility.DetailedLogDepth;
			LogCurrentPlayerLoopTypes(message, sourceDepth, logType);
		}
		...
		public static void SubscribeToLoop(Type loopType, PlayerLoopSystem.UpdateFunction updateFunction)
		{
			...
			{
				var message = $"Not found {nameof(loopType)}({loopType.Name}) for subscribing";
		...
		public static void AddSubLoop(Type sourceLoopType, Type destinationLoopType, PlayerLoopSystem.UpdateFunction updateFunction)
		{
			...
			{
				sourceSystem.subSystemList = new[]
				{
					destinationSystem,
				};
			}
		...
		private static void InsertLoopWithOffset(Type sourceLoopType, Type destinationLoopType, int insertOffset, PlayerLoopSystem.UpdateFunction updateFunction)
		{
			...
			{
				parentSystem.subSystemList = new[]
				{
					destinationSystem,
				};
			}
